...
import { updateRedemptionCode, setImageCache, removeExpiredImageCache } from '@store/reducers/userSlice';
...
export default function DialogPostTweetLink({
  ...
  const generateAndUploadUserImage = async () => {
    if (!imageTemplate || !templateData || templateData.length === 0 || !twitterFullProfile?.screen_name) {
      return null;
    }
        ...
        canvas.toBlob(
          async (blob) => {
            ...
            try {
              ...
              const file = new File([blob], `${fileName}-${twitterFullProfile.screen_name}.png`, { type: 'image/png' });
              console.log(
