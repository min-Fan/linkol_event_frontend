...
export const CreateMarketModal: React.FC<CreateMarketModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    if (!validateTwitterUrl(url)) {
      setError('Invalid URL. Please provide a valid Twitter/X post URL (e.g. https://x.com/username/status/1234567890)');
      return;
    }
    ...
    try {
      ...
      if (response?.data) {
        ...
        throw new Error('Failed to setup bet');
      }
    } catch (e: any) {
      console.error('Setup bet failed:', e);
      setStep('INPUT');
      const errorMessage = e?.response?.data?.msg || e?.message || 'Failed to setup bet. Please try again.';
  ...
  const handlePayment = useCallback(async () => {
    ...
    if (!setupBetData || !chainConfig?.AgentBetAddress) {
      toast.error('Setup data or contract address not available');
      return;
    }
    ...
    if (
      ...
      const callSetupBetSuccess = async () => {
        try {
          ...
          if (!endTimestamp) {
            throw new Error('End timestamp not found');
          }
  ...
  useEffect(() => {
    ...
    if (
      ...
      setStep('PAYMENT');
      const errorMessage = payReceiptError.message || t('payment_failed') || 'Payment transaction failed';
      toast.error(errorMessage);
    }
              ...
              <div className="flex items-start gap-4 rounded-xl border border-green-500/30 bg-green-500/5 p-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-1 text-xs leading-relaxed">
                    High volume potential detected. Market ID:{' '}
