...
export default function ChatKeyKolsComparison() {
  ...
  const handleSubmit = async () => {
    ...
    // 验证两个KOL名称不能相同
    if (
      chat_comparison_info.kol1.name.trim().toLowerCase() ===
      chat_comparison_info.kol2.name.trim().toLowerCase()
    ) {
      toast.error(t('keyword_kol_duplicate_error'));
      return;
    }
  ...
  const query = (data: any, index: number) => {
    ...
    if (index == 1) {
      // 检查KOL1是否与当前的KOL2重复
      if (
        chat_comparison_info.kol2.name &&
        data.username.toLowerCase() === chat_comparison_info.kol2.name.toLowerCase()
      ) {
        toast.error(t('keyword_kol_duplicate_error') || '不能选择相同的KOL进行比较');
        return;
      }
    ...
    if (index == 2) {
      // 检查KOL2是否与当前的KOL1重复
      if (
        chat_comparison_info.kol1.name &&
        data.username.toLowerCase() === chat_comparison_info.kol1.name.toLowerCase()
      ) {
        toast.error(t('keyword_kol_duplicate_error') || '不能选择相同的KOL进行比较');
        return;
      }
  ...
  return (
    <Popover open={chat_comparison_view} onOpenChange={setIsOpen}>
      ...
      <PopoverContent
        className="w-62 sm:w-96"
        onInteractOutside={(e) => !pathname.includes('chat') && e.preventDefault()}
      >
