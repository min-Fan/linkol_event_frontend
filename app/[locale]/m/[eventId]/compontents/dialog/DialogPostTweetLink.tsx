...
export default function DialogPostTweetLink({
  ...
  const { isAccepted: isAgentAccepted, isLoading: isAgentLoading } = useAgentStatus();
    ...
    try {
      ...
      if (eventInfo?.a_type === 'platform') {
        ...
        res = await sendActivityTweetPlatform({
          ...
          language: LanguageCodeShort[selectedLanguage], // 可以根据需要调整
  ...
  const handleLinkAgentClose = useCallback(() => {
    setIsLinkAgentOpen(false);
  }, []);

  const handleLinkAgentSuccess = useCallback(() => {
    setIsLinkAgentOpen(false);
    // Agent 创建成功后可以执行其他操作
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
                        ...
                        <DropdownMenu>
                          ...
                          >
                            ...
                            >
                              <span className="text-sm">
                                ...
                                {selectedLanguage === LanguageCode.Korea && t('korean')}
                              </span>
                          ...
                          >
                            {[
                              ...
                              { code: LanguageCode.Korea, label: t('korean') },
                            ].map((lang) => (
                              <DropdownMenuItem
                                key={lang.code}
                                onClick={() =>
      ...
      {/* Link Agent 弹窗 */}
      <DialogLinkAgent
        isOpen={isLinkAgentOpen}
        onClose={handleLinkAgentClose}
        onSuccess={handleLinkAgentSuccess}
        eventInfo={eventInfo}
      />
      {/* 隐藏的模板组件用于生成用户自己的图片 */}
      {imageTemplate &&
